...
import TopArticle from '../components/Politics/TopArticle';
import MostPopular from '../components/Politics/MostPopular';
...
function Politics() {
    ...
    useEffect(() => {
        const getArticles = async () => {
            setLoading(true);
            const res = await axios.get(` https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=section_name:("Sports")&sort=newest&api-key=${process.env.REACT_APP_NYTIMES_API_KEY}`);
    ...
    return (
        ...
        <div className="flex z-50 top-20  lg:justify-between items-center">
            <h1 className="text-4xl">Politics</h1>
